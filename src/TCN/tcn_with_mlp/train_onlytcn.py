...
def train():
    ...
    hidden_size=64
    num_layers=3
    num_channels = [16,16,3] # args.tcn_channels
    kernel_size = 3
    dropout = 0.3

    model = TCN(input_size=input_size, output_size=output_size, num_channels=[16,16,3], kernel_size=kernel_size, dropout=dropout)
    # num_inputs:特征数
    ...
    for epoch in bar:
        for input_data, target_data in data_loader:
            # input data shape is (batchsize, length, channels)
            optimizer.zero_grad()
            output = model(input_data)
            # print(f"input data:{input_data.shape}\n")
            # print(f"out value: {target_data.shape}\n")
