...
parser.add_argument('--n_input', type=int, default=7,
...
def calc_diff_time(t, start_dice):
    ...
    #     'dt': raw_dt
    # })
    # data.to_csv(os.path.join(cur_path+'/data/test/dt.csv'))
    return raw_dt
...
def train():
    ...
    # print(f'raw_dt:{raw_dt.shape}')
    # print(f'raw_x_dot{raw_x_dot.shape}')
    # time_sequence = np.column_stack((raw_x, raw_y, raw_yaw, raw_v, raw_w))
    time_sequence = np.column_stack((raw_x_dot, raw_y_dot, raw_yaw_dot, raw_u_v, raw_u_w, raw_yaw, raw_dt)) # with control
    ...
    save_file_path = os.path.join(cur_path + '/../results/tcn_withyaw_dt1.pt')
