...
array_data = array_data.reshape(40,1)
# Dimensions
nx = 40
nu = 2
ny = nx + nu
...
cost_W = scipy.linalg.block_diag(Q, R) # ny*ny
cost_W_e = Q # nx*nx 

cost_Vx = B
cost_Vu = np.zeros((ny, nu))
cost_Vu[-nu:, -nu:] = np.eye(nu)
# ocp.cost.Vz = np.array([[]])
cost_Vx_e = A  

np.set_printoptions(threshold=np.inf)
print(f'should be output:{0.010920714489099,0.005126101958987,0.6}')
print(f'x shape:{(cost_Vx_e@array_data)}')
print(f'Vxe*x:{(cost_Vx_e@array_data).T@cost_W_e@(cost_Vx_e@array_data)}')
