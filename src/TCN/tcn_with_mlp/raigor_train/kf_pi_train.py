...
def physics_loss(out, input):
    ...
    return loss
# has not done yet
def kalman_filter(input, out, x_init, P_init, measurements):
    """
    简单的卡尔曼滤波器
    ...
    返回：
    - state_estimates: 估计的状态序列
    - P_estimates: 估计的状态协方差矩阵序列
    """
    v_in = input[:, -1, 3]
    w_in = input[:, -1, 4]
    theta_in = input[:, -1, 5]
    ...
    T = 0.01 # need to change
    # 定义系统参数
    A = np.array([
        [1, 0, -T * v_in * torch.sin(theta_in)],
        [0, 1, v_in * torch.cos(theta_in)],
        [0, 0, 1]
    ])  # 状态转移矩阵，由于速度直接通过控制输入来更新，因此为单位矩阵
    B = np.array([
        [torch.cos(theta_in), 0, 0],
        [0, torch.sin(theta_in), 0],
        [0, 0, 1]
    ])  # 控制输入矩阵，由于速度直接通过控制输入来更新，因此根据模型中的关系填充
    H = np.eye(3)  # 测量矩阵
    Q = np.eye(3) * 0.01  # 状态转移过程噪声协方差
    ...
    for z in measurements:
        ...
        P_est = (np.eye(num_states) - K.dot(H)).dot(P_pred)

        # 存储估计结果
        state_estimates.append(x_est)
        P_estimates.append(P_est)

    return np.array(state_estimates), np.array(P_estimates)
