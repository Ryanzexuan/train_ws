...
from tcn_common import TCNModel, TemporalConvNet, TemporalBlock, Chomp1d, get_training_datasets
# 数据准备（示例数据，你需要替换为实际数据）
# 假设你有一个包含历史序列和目标序列的示例数据
# 这里使用随机生成的示例数据，需要根据你的数据进行替换
...
data = np.random.rand(data_length, sequence_length, 3).astype(np.float32)
# 迭代处理原始数据
j = 0
last_idx = 0
for i in idx[0]:
...
input_size = 3
output_size = 3
num_channels = [64, 64, 64]  # 通道数可以根据需求进行调整
kernel_size = 2
dropout = 0.2
...
with torch.no_grad():
    ...
    input_sequence_tensor = input_sequence[0:1]  # 选择第一个序列作为示例
...
# 绘制实际数据和预测数据
actual_data = target_sequence[0].numpy()  # 实际数据
predicted_data = predicted_sequence[0].numpy()  # 预测数据

plt.subplot(2, 1, 2)  # 第二个子图用于实际和预测数据
...
plt.ylabel("Value")
plt.title("Actual vs. Predicted Data (x, y, z)")
plt.legend()
