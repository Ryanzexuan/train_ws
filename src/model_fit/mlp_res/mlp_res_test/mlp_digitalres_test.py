...
import sys
sys.path.append('/home/ryan/raigor/train_ws/src/model_fit/mlp_res/mlp_res_test')
import os
import time
import argparse
import subprocess
...
def data_pre_process(data):
    ...
    # df_val_pd = df_val_pd.reset_index(drop=True)
    # print(f"df_val_p:{df_val_pd}")
    # print(f"data:{data}")
    df_new = pd.concat([data, df_val_pd], axis=1)
    # print(f"df_new:{df_new}")
    # print(f"vel_w_out:{df_new['vel_w_output']}")
    return df_new
...
def main(x_features, u_features, reg_y_dims, model_ground_effect, quad_sim_options, dataset_name,
    ...
    save_file_path = os.path.join("/home/ryan/raigor/train_ws/results/model_fitting/", str("res"), str(gp_name_dict["model_name"]))
    ...
    input_dims = 5  # MLP input dimension
